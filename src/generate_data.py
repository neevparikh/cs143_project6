...
def main():
    ...
    parser.add_argument("--regen", choices=["true,false"], default='false', required=True)
    ...
    data = get_pose_normed_estimate(source, target, regen=regen, rotate=rotated,
    ...
    for i in range(len(source_frames)):
        ...
        if phase =='train':
            ...
            target_image = draw_bodypose(canvas, target_poses[i], target_subsets[i])
            target_image = cv2.cvtColor(target_image, cv2.COLOR_BGR2GRAY)

            cv2.imwrite('data/train_label/target_pose_' + str(frame_counter) + '.png', target_image)
            cv2.imwrite('data/train_img/target_frame_' + str(frame_counter) + '.png', target_frames[i])

        elif phase == 'test':
            
            source_image = draw_bodypose(canvas, normed_source[i], source_subsets[i])
            source_image = cv2.cvtColor(source_image, cv2.COLOR_BGR2GRAY)
            
            cv2.imwrite('data/test_label/source_pose_' + str(frame_counter) + '.png', source_image)
            cv2.imwrite('data/test_img/source_frame_' + str(frame_counter) + '.png', source_frames[i])
